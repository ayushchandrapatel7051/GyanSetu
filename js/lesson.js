...
=======
  ...
  let savedProgress = await LessonDB.getProgress(email, lessonId).catch(() => null);
  if (!savedProgress) {
    savedProgress = {
      id: idKey,
      email,
  ...
  async function renderLesson(lang) {
    ...
    metaContainer.innerHTML = `${view.subject} • Grade ${view.grade} • ${view.duration_minutes || "—"} mins`;
      ...
      switch (block.type) {
        ...
        case "image":
          wrapper.innerHTML = `<figure>
            <img src="${block.src}" alt="${escapeHTML(block.alt || "")}" style="max-width:100%;border-radius:8px"/>
        ...
        case "video":
          ...
          break;
        case "list":
          wrapper.innerHTML = `<${block.ordered ? "ol" : "ul"}>${(block.items || []).map(i => `<li>${escapeHTML(i)}</li>`).join("")}</${block.ordered ? "ol" : "ul"}>`;
          break;
      }
    ...
    tickTimer = setInterval(() => {
      const total = savedProgress.timeSpentSeconds + Math.floor((Date.now() - startedAt) / 1000);
  ...
  document.addEventListener("gyan:user-updated", async () => renderLesson(await resolveLanguage()));
  window.addEventListener("storage", async () => renderLesson(await resolveLanguage()));
  ...
  bookmarkBtn.addEventListener("click", () => {
    ...
    bookmarkBtn.textContent = arr.includes(lessonId) ? "Bookmarked ✓" : "Bookmark";
  });
...
>>>>>>> 0e8109c14c062bc5278e5aeaec40dfe75d09cb3f
